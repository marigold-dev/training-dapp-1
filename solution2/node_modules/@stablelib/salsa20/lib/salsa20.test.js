...
describe("salsa20.streamXOR", function () {
    var key = seq(32);
    var nonce8 = seq(8);
    it("should decrypt what it encrypted (17-byte input)", function () {
        var original = seq(17);
        var encrypted = new Uint8Array(original.length);
        var decrypted = new Uint8Array(original.length);
        salsa20_1.streamXOR(key, nonce8, original, encrypted);
        expect(encrypted).not.toEqual(original);
        salsa20_1.streamXOR(key, nonce8, encrypted, decrypted);
        expect(decrypted).toEqual(original);
    });
    it("should decrypt what it encrypted (155-byte input)", function () {
...
describe("salsa20.stream", function () {
    it("should generate test vectors", function () {
        vectors.forEach(function (v) {
            var key = hex_1.decode(v.key);
            var nonce = hex_1.decode(v.nonce);
            var dst = salsa20_1.stream(key, nonce, new Uint8Array(v.length));
            var xorDigest = new Uint8Array(64);
            var i = 0;
            while (i < dst.length) {
                for (var j = 0; j < 64; j++) {
                    xorDigest[j] ^= dst[i++];
            ...
            expect(hex_1.encode(xorDigest)).toBe(v.xorDigest);
        });
    });
});
// TODO(dchest): test nonceInplaceCounterLength
//# sourceMappingURL=salsa20.test.js.map